...
export function PublicCollectionItem({ link, allLinks, theme, themeConfig, linkStyle, accentColor, openForm, onLinkClick, isPreview = false }: PublicCollectionItemProps) {
    ...
    const headerPadding = isPreview ? 'p-3' : (theme === 'neo' ? 'p-6' : 'p-4');
    const contentPadding = isPreview ? 'p-2' : 'p-4';
    const contentNeoPadding = isPreview ? 'px-3 pb-3' : 'px-6 pb-6';
    const gap = isPreview ? 'gap-2' : 'gap-3';
    ...
    return (
        <div className={wrapperClass} style={!themeConfig ? dynamicStyle : {}}>
            ...
            <button
                ...
                className={`w-full text-left flex items-center justify-between group/header outline-none ${headerPadding}`}
            >
                <div className="flex items-center gap-2">
                    <div className={`transition-transform duration-300 ${isOpen ? 'rotate-90' : 'rotate-0'}`}>
                        {theme === 'neo' ? (
                            <ChevronRight size={isPreview ? 16 : 24} strokeWidth={3} />
                        ) : (
                            <ChevronRight size={isPreview ? 14 : 20} />
                        )}
                    </div>
                    <div>
                        <h3 className={`font-bold leading-tight ${theme === 'neo' ? (isPreview ? 'text-xs uppercase' : 'text-xl uppercase') : (isPreview ? 'text-sm' : 'text-lg')}`}>
                            {link.title}
                        </h3>
                        {collectionData.description && (
                            <p className={`mt-0.5 opacity-70 ${theme === 'noir' ? 'font-light' : ''} ${isPreview ? 'text-[10px]' : 'text-sm'}`}>
                                {collectionData.description}
                            </p>
                        )}
                    </div>
                ...
                {childLinks.length > 0 && (
                    <span className={`font-medium opacity-60 bg-black/10 dark:bg-white/10 rounded-full ${isPreview ? 'text-[10px] px-1.5 py-0.5' : 'text-xs px-2 py-1'}`}>
                        {childLinks.length}
                    </span>
                )}
            </button>
            ...
            <div
                ...
                `}
            >
                <div className={`flex flex-col ${gap} ${contentPadding} pt-0 ${theme === 'neo' ? `${contentNeoPadding} border-t-2 border-black` : ''}`}>
